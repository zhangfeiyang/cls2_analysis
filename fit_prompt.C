...
//	t->Add("/junofs/production/public/users/zhangfy/non-uniform/offline_J17v1r1-Pre1/Examples/Tutorial/share/cls2/Co60/10000_10000/evt_*root");
	...
	t->Draw("promptPE>>h(200,7000,10000)","promptPE>0","",total_entries,total_entries-max_entries);
	TH1F *h = (TH1F*)gDirectory->Get("h");

    int maxbin = h->GetMaximumBin();
    double maxbincenter = h->GetBinCenter(maxbin);

    t->Draw(Form("promptPE>>h(100,%i,%i)",int(maxbincenter-100),int(maxbincenter+100)),"","",total_entries,total_entries-max_entries);
	...
	t->Draw(Form("promptPE>>h(%i,%i,%i)",int(5*sigma),int(mean-5*sigma),int(mean-5*sigma)+int(5*sigma)*2),"","",total_entries,total_entries-max_entries);
    h = (TH1F*)gDirectory->Get("h");
    if(R=="16000" || R=="17000" || R=="17400"){
		t->Draw(Form("promptPE>>h(%i,%i,%i)",int(6*sigma),int(mean-7*sigma),int(mean-7*sigma)+int(6*sigma)*2),"","",total_entries,total_entries-max_entries);
	    h = (TH1F*)gDirectory->Get("h");
	...
	filename = dirname+"result_prompt.C";
	c1->SaveAs(&filename[0]);
	filename = dirname+"result_prompt.png";
	c1->SaveAs(&filename[0]);

	filename = dirname+"result_prompt";
