...
//	t->Add("/junofs/production/public/users/zhangfy/non-uniform/offline_J17v1r1-Pre1/Examples/Tutorial/share/cls2/Co60/10000_10000/evt_*root");
	...
	t->Draw("delayPE>>h(200,2500,3500)","delayPE>0","",total_entries,total_entries-max_entries);
	...
	t->Draw(Form("delayPE>>h(%i,%i,%i)",int(5*sigma),int(mean-5*sigma),int(mean-5*sigma)+int(5*sigma)*2),"","",total_entries,total_entries-max_entries);
    h = (TH1F*)gDirectory->Get("h");
	...
	filename = dirname+"result_nH.C";
	c1->SaveAs(&filename[0]);
	filename = dirname+"result_nH.png";
	c1->SaveAs(&filename[0]);

	filename = dirname+"result_nH";
