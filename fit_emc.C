...
double myfun(double *xx, double *par)
{
    ...
    double constant = (1-alpha)/mu*(TMath::Erf((mu-E)/sqrt(2)/sigma)-TMath::Erf(-E/sqrt(2)/sigma));

    return c*(gaus + constant);
}
...
bool get_data(int argc,char **argv,double *pars,double *epars,double& chi2,double &ndf,string dirname,int index)
{
    ...
    TFile *file;
    TF1 *f = new TF1("f",myfun,0,20000,4);
...
*/	h->Fit("gaus","Q");
	...
	memcpy(epars,tmps,4*sizeof(double));
